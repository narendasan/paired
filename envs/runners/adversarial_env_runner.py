...
class AdversarialEnvRunner(object):
    """
    ...
    def _get_env_stats_multigrid(self, agent_info):
    ...
    def _get_env_stats_minihack(self, agent_info):
        stats = self._get_env_stats_multigrid(agent_info)
    ...
    def _get_env_stats(self, agent_info):
        env_name = self.args.env_name
        if env_name.startswith('MultiGrid'):
            stats = self._get_env_stats_multigrid(agent_info)
        elif env_name.startswith('MiniHack'):
            stats = self._get_env_stats_minihack(agent_info)
        else:
    ...
    def agent_rollout(self,
        ...
        if not is_env:
            ...
            value_loss, action_loss, dist_entropy, info = agent.update(no_grad=(not self.is_training))
            print(value_loss, action_loss)
    ...
    def _compute_env_return(self, agent_info):
        args = self.args

        if args.ued_algo == 'paired':
            env_return = torch.max(agent_info['mean_return'], torch.zeros_like(agent_info['mean_return']))
            ...
            raise RuntimeError("flexible paired not implemented")
    ...
    def run(self):
        ...
        env_return = self._compute_env_return(agent_info)
        ...
        if self.is_training and self.is_training_env:
            ...
            env_value_loss, env_action_loss, env_dist_entropy, info = adversary_env.update(no_grad=self.is_training)
        ...
        # === LOGGING ===
        stats = self._get_env_stats(agent_info)
        stats.update({
        ...
        #     print(adversary_env_info['update_info']['grad_norms'])
